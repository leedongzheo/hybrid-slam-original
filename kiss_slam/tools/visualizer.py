...
class RegistrationVisualizer(StubVisualizer):
    def __init__(self):
        ...
        self.current_marker_pose = None
        # self.odom_pts = deque(maxlen=2)
        self.odom_pts = []                 # <-- Sửa thành list trống
        # --- THÊM DÒNG NÀY ---
        self.traj_line_set = self.o3d.geometry.LineSet()
        self._traj_line_added = False
        # ---------------------
        self.vis = self.o3d.visualization.VisualizerWithKeyCallback()
        self._register_key_callbacks()
        self._initialize_visualizer()
    ...
    def _update_odom_tubes(self, current_pose, z_anchor, radius=0.5):
        ...
        if len(self.odom_pts) == 0 or np.linalg.norm(pt - self.odom_pts[-1]) > 1e-9:
            ...
            # 1. Vẽ ống 3D (Mesh Cylinder) như cũ
            if len(self.odom_pts) >= 2:
            ...
            # 2. CẬP NHẬT LINESET (Lớp bảo vệ khi zoom xa)
            if len(self.odom_pts) > 1:
                pts_array = np.array(self.odom_pts)
                # Nâng nhẹ trục Z lên 0.5m để đường line nổi lên trên mặt map
                pts_array[:, 2] += 0.5 
                
                lines = [[i, i+1] for i in range(len(pts_array)-1)]
                colors = [self.theme["trajectory"] for _ in range(len(lines))]
                
                self.traj_line_set.points = self.o3d.utility.Vector3dVector(pts_array)
                self.traj_line_set.lines = self.o3d.utility.Vector2iVector(lines)
                self.traj_line_set.colors = self.o3d.utility.Vector3dVector(colors)
                
                if not self._traj_line_added:
                    self.vis.add_geometry(self.traj_line_set, reset_bounding_box=False)
                    self._traj_line_added = True
                else:
    ...
    def _capture_rgb_frame(self):
        ...
        # --- FIX LỖI MP4: Ép kích thước khung hình luôn là số chẵn ---
        h, w = img_u8.shape[:2]
        img_u8 = img_u8[:h - (h % 2), :w - (w % 2), :]
        # -------------------------------------------------------------
    ...
    def _update_geometries(self, slam):
        ...
        if key_poses != self.key_poses:
            ...
            for e in self.edges: self.vis.add_geometry(e, reset_bounding_box=False)
        # --- [FIX] TỰ ĐỘNG ZOOM OUT BAO QUÁT TOÀN BẢN ĐỒ ---
        # if self.reset_bounding_box:
        #     self.vis.reset_view_point(True)
        #     self.reset_bounding_box = False
        
        # # Nếu đang ở chế độ Toàn cảnh (Không Follow Cam), 
        # # Cứ mỗi 30 frames, ta ép camera lùi ra xa để thấy trọn vẹn bản đồ đang to dần
        # elif not self.follow_cam and self.frame_idx % 300 == 0:
        #     self.vis.reset_view_point(True)
        # --- [FIX] TỰ ĐỘNG ZOOM OUT THEO KỊCH BẢN ĐỊNH SẴN ---
        # Danh sách các frame muốn kích hoạt zoom
        # zoom_frames = [300, 1500, 2000, 4000]
        # if self.reset_bounding_box:
        #     self.vis.reset_view_point(True)
        #     self.reset_bounding_box = False
        
        # # Nếu đang ở chế độ Toàn cảnh (Không Follow Cam) và chạm đúng frame chỉ định
        # elif not self.follow_cam and self.frame_idx in zoom_frames:
        #     self.vis.reset_view_point(True)
        #     # In ra Terminal để bạn dễ theo dõi tiến độ
        #     print(f"[Camera] Đã tự động Zoom Out tại frame: {self.frame_idx}")
        if self.reset_bounding_box:
            self.vis.reset_view_point(True)
            self.reset_bounding_box = False
